...
window.onload = function init()
{
    ...
    translationLoc = gl.getUniformLocation(g_program, "translation");
    gl.uniform3fv(translationLoc, flatten(translationVector));
    qLoc = gl.getUniformLocation(g_program, "q");
    qInverseLoc = gl.getUniformLocation(g_program, "qInverse");
    viewMatrixLoc = gl.getUniformLocation(g_program, "viewMatrix");
    projectionMatrixLoc = gl.getUniformLocation(g_program, "projectionMatrix");
    var colorStyleUseXYZLoc = gl.getUniformLocation(g_program, "colorStyleUseXYZ");
    gl.uniform1i(colorStyleUseXYZLoc, 0);
    ...
    maxesLoc = gl.getUniformLocation(g_program, "maxes");
    gl.uniform3fv(maxesLoc, [fractal.max.x, fractal.max.y, fractal.max.z]);
    ...
    checkbox.onclick = function(e) {
        if (checkbox.checked == true) {
            gl.uniform1i(colorStyleUseXYZLoc, 1);
        } else {
            gl.uniform1i(colorStyleUseXYZLoc, 0);
        }
    }
    ...
    lookButton.onclick = function () {
        ...
        current_q = null;
        gl.uniform4fv(qLoc, flatten([0.0, 0.0, 0.0, 0.0]));
...
function zoom_update() {
    if (zooming && mouse_changed) {
        ...
        gl.uniformMatrix4fv(viewMatrixLoc, false, flatten(matrix));
        modelView = matrix;
...
function rotation_update() {
    if (clicking && mouse_changed) {
        ...
        gl.uniform4fv(qLoc, current_q.array());
        gl.uniform4fv(qInverseLoc, current_q.inverse().array());
        clicked_position = mouse_position;
        mouse_changed = false;
    }
}
